...
local function checkAvailability()
	...
	local exitCode = noctalia.runSync(
		"command -v flatpak >/dev/null 2>&1 && flatpak list --app 2>/dev/null | grep -q com.dec05eba.gpu_screen_recorder"
	)
	return exitCode == 0
end
...
local function startRecording()
	if not isAvailable or state ~= "idle" then
		return
	end
...
local function stopRecording()
	if state ~= "recording" and state ~= "pending" then
		return
	end

	noctalia.runAsync(
		"pkill -SIGINT -f '^(/nix/store/.*-)?gpu-screen-recorder' 2>/dev/null || pkill -SIGINT -f '^com.dec05eba.gpu_screen_recorder' 2>/dev/null || true"
	)
	-- Force kill fallback
...
local function startReplay()
	...
	if not cfg("replay_enabled", false) then
		return
	end
...
local function stopReplay()
	if state ~= "replaying" and state ~= "replay_pending" then
		return
	end
